...
export class LecturerDashboardComponent implements OnInit {
    ...
    constructor(
        ...
        private http: HttpClient // <-- INIETTATO HTTPCLIENT
    ) {}
    ...
    caricaPratiche() {
        ...
        this.http.get<any[]>(`http://localhost:3000/api/lecturer/applications?email=${this.utente.email}`).subscribe({
            next: (data) => {
            ...
            error: (err) => console.error('Errore recupero pratiche docente:', err),
        });
    }
    ...
    startReview(app: any, docType: 'LEARNING_AGREEMENT' | 'TRANSCRIPT_OF_RECORDS') {
        ...
        this.http.get<any>(`http://localhost:3000/api/applications/${app.id}`).subscribe({
            ...
            error: (err) => console.error('Errore durante la preparazione della review:', err),
        });
    }
    ...
    eseguiAzione() {
        if (this.reviewData.isAcceptingDraft) {
            ...
            this.http.put(`http://localhost:3000/api/lecturer/applications/${this.reviewData.appId}/draft-review`, payloadDraft).subscribe({
                next: () => {
                    this.modalStep = 'success';
                    this.caricaPratiche();
                    this.cdr.detectChanges();
                },
                error: (err) => console.error('Errore server durante draft review:', err),
            });
        ...
        this.http.put(`http://localhost:3000/api/lecturer/applications/${this.reviewData.appId}/review`, payloadReview).subscribe({
            next: () => {
                this.modalStep = 'success';
                this.caricaPratiche();
                this.cdr.detectChanges();
            },
            error: (err) => console.error('Errore server durante review ufficiale:', err),
        });
    }
    ...
    openHistoricalDetails(app: any) {
        this.http.get<any>(`http://localhost:3000/api/applications/${app.id}`).subscribe({
            ...
            error: (err) => console.error('Errore recupero dettagli storici:', err),
        });
    }
